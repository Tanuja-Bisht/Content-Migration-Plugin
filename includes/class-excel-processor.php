...
class Excel_Processor {
    ...
    private function process_csv_file($file_path, $allow_overwrite) {
        ...
        foreach ($rows as $index => $row_data) {
            ...
            // ADDED: Skip if we've already processed this title in the current batch
            $title = isset($row_data['title']) ? trim($row_data['title']) : '';
            if (!empty($title) && in_array($title, $processed_titles)) {
                error_log("MIGRATION: Skipping duplicate title in same import batch: " . $title);
                $result = array(
                    'status' => 'skipped',
                    'message' => "Duplicate title in same import batch: '{$title}'. Skipped.",
                    'title' => $title,
                    'slug' => isset($row_data['slug']) ? $row_data['slug'] : '',
                    'url' => isset($row_data['new_url']) ? $row_data['new_url'] : ''
                );
            } else {
                // Process the row
                $result = $this->process_row($row_data, $allow_overwrite, true);
                
                // If successful, add the title to our processed list
                if ($result['status'] === 'success' && !empty($title)) {
    ...
    private function process_row($row_data, $allow_overwrite, $skip_rewrite_flush = false) {
        ...
        // ADDED: Check by title first to prevent duplicates
        if (!empty($page_title)) {
            global $wpdb;
            $existing_post_id = $wpdb->get_var($wpdb->prepare(
                "SELECT ID FROM $wpdb->posts WHERE post_title = %s AND post_type = %s AND post_status IN ('publish', 'draft')",
                $page_title,
                $type
            ));
            ...
            if ($existing_post_id) {
                ...
                $existing_url = '/' . get_page_uri($existing_post_id);
                error_log("MIGRATION: Found existing post with same title: {$page_title} (ID: {$existing_post_id}, URL: {$existing_url})");
                
                return array(
                    'status' => 'skipped',
                    'message' => "Content with same title already exists: '{$page_title}' (ID: {$existing_post_id}). Skipped.",
                    'title' => $page_title,
                    'slug' => isset($existing_post) ? $existing_post->post_name : ''
                );
            }
        }
        ...
        // MODIFIED: Always skip if existing post is found, regardless of allow_overwrite setting
        if ($existing_post) {
            return array(
                'status' => 'skipped',
    ...
    private function find_existing_post_by_path($path, $post_type) {
        ...
        // ADDED: More aggressive check - just look for the slug directly in any post
        global $wpdb;
        $existing_post_id = $wpdb->get_var($wpdb->prepare(
            "SELECT ID FROM $wpdb->posts WHERE post_name = %s AND post_type = %s AND post_status IN ('publish', 'draft') LIMIT 1",
            $slug,
            $post_type
        ));
        
        if ($existing_post_id) {
            error_log("MIGRATION: Found existing post with matching slug: {$slug} (ID: {$existing_post_id})");
            return get_post($existing_post_id);
        }
    ...
    private function extract_first_image_as_featured($content, $post_id) {
        ...
        if ($featured_image_set) {
            // Check if the image is wrapped in a paragraph, div, or figure
            $parent_tag_pattern = '/<(?:p|div|figure)[^>]*>\s*' . preg_quote($first_img_tag, '/') . '\s*<\/(?:p|div|figure)>/is';
            if (preg_match($parent_tag_pattern, $updated_content, $parent_matches)) {
                ...
                // Check specifically for WordPress caption markup
                $caption_pattern = '/\[caption[^\]]*\]' . preg_quote($first_img_tag, '/') . '.*?\[\/caption\]/is';
                if (preg_match($caption_pattern, $updated_content, $caption_matches)) {
                    // Remove the entire caption shortcode
            ...
            $updated_content = preg_replace('/<p>\s*<\/p>/is', '', $updated_content);
        }
    ...
    private function remove_duplicate_featured_image($content, $post_id) {
        ...
        foreach ($img_tags as $index => $img_tag) {
            ...
            if ($is_duplicate) {
                // Check if this image is wrapped in a paragraph, div or figure
                ...
                if (preg_match($parent_tag_pattern, $content, $parent_matches)) {
                    ...
                    if (preg_match($caption_pattern, $content, $caption_matches)) {
                        // Remove the entire caption shortcode
                        $content = str_replace($caption_matches[0], '', $content);
                        error_log("MIGRATION: Removed duplicate featured image with caption shortcode from content. Image: {$img_url}");
                    } else {
                        // Just remove the image tag
